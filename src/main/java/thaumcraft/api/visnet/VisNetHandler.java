...
public class VisNetHandler
{
	...
	{
		HashMap<WorldCoordinates, WeakReference<TileVisNode>> sourcelist = sources.get(Integer.valueOf(world.provider.dimensionId));
		if (sourcelist == null)
		...
		else
		{
			...
			for (WeakReference<TileVisNode> root : sourcelist.values())
				if (isNodeValid(root))
				{
					...
					if (r > 0.0F)
						nearby.add(new Object[] { source, Float.valueOf(r - vn.getRange() * 2) });
			...
			if (nearby.size() > 0)
				for (Object[] o : nearby)
					if (((Float) o[1]).floatValue() < dist && (vn.getAttunement() == -1 || ((TileVisNode) o[0]).getAttunement() == -1 || vn.getAttunement() == ((TileVisNode) o[0]).getAttunement()) && canNodeBeSeen(vn, (TileVisNode) o[0]))
					{
	...
	private static final Set<TileVisNode> tiles = Collections.newSetFromMap(new WeakHashMap<TileVisNode, Boolean>());
	...
	{
		return getAllChildren(true, source, list);
	}
	// TODO gamerforEA code end

	// TODO gamerforEA add boolean parameter
	private static ArrayList<WeakReference<TileVisNode>> getAllChildren(boolean clear, TileVisNode source, ArrayList<WeakReference<TileVisNode>> list)
	{
		// TODO gamerforEA code start
		if (!tiles.add(source))
			return list;
		...
		for (WeakReference<TileVisNode> child : source.getChildren())
		{
			...
			if (n != null && n.getWorldObj() != null && isChunkLoaded(n.getWorldObj(), n.xCoord, n.zCoord))
			{
				...
				// TODO gamerforEA add boolean parameter
				list = getAllChildren(false, n, list);
			}
		}
