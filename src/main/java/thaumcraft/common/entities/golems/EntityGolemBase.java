...
public class EntityGolemBase extends EntityGolem implements IEntityAdditionalSpawnData
{
	...
	{
		return MathHelper.floor_double(Math.sqrt((double) this.getCarryLimit())) * 1000;
	}
	...
	public void onLivingUpdate()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			if (this.getDistanceSq((double) this.getHomePosition().posX, (double) this.getHomePosition().posY, (double) this.getHomePosition().posZ) >= 2304.0D || this.isEntityInsideOpaqueBlock())
			{
	...
	protected void damageEntity(DamageSource ds, float par2)
	{
		if (!ds.isFireDamage() || !this.golemType.fireResist)
		{
			...
			{
				this.setLocationAndAngles((double) this.getHomePosition().posX + 0.5D, (double) this.getHomePosition().posY + 0.5D, (double) this.getHomePosition().posZ + 0.5D, 0.0F, 0.0F);
			}
	...
	public void updateCarried()
	{
		...
		{
			this.getDataWatcher().updateObject(16, new ItemStack(Item.getItemById(this.fluidCarried.fluidID), 1, this.fluidCarried.amount));
			this.getDataWatcher().setObjectWatched(16);
		}
