...
public class ItemFocusShock extends ItemFocusBasic
{
		...
		if (this.isUpgradedWith(wand.getFocusItem(itemstack), earthshock))
		{
			if (wand.consumeAllVis(itemstack, p, this.getVisCost(itemstack), !p.worldObj.isRemote, false))
			{
				if (!world.isRemote)
				{
					...
					orb.damage = (int) ((double) orb.damage + (double) wand.getFocusPotency(itemstack) * 1.33D);
	...
	public void doLightningBolt(ItemStack stack, EntityPlayer p, int count)
	{
		...
		else
		{
			...
			{
				MovingObjectPosition cl = BlockUtils.getTargetBlock(p.worldObj, p, false);
				...
				if (cl != null)
				{
					targets = cl.hitVec.xCoord;
					d = cl.hitVec.yCoord;
					closest = cl.hitVec.zCoord;
				...
				if (pointedEntity != null)
				{
					...
					d = pointedEntity.boundingBox.minY + (double) (pointedEntity.height / 2.0F);
					...
					if (var18 > 0)
					{
						...
						while (var18 > 0)
						{
							...
							if (var21 != null)
							{
								...
								var21.attackEntityFrom(DamageSource.causePlayerDamage(p), (float) (4 + potency));
