...
public class WarpEvents
{
	...
	private static void grantResearch(EntityPlayer player, int times)
	{
		...
		for (int a = 0; a < amt; ++a)
		{
			...
			PacketHandler.INSTANCE.sendTo(new PacketAspectPool(aspect.getTag(), Short.valueOf((short) 1), Short.valueOf(Thaumcraft.proxy.playerKnowledge.getAspectPoolFor(player.getCommandSenderName(), aspect))), (EntityPlayerMP) player);
		}
