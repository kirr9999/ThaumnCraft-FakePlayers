...
public class ExplosionByPlayer extends Explosion
{
	...
	public void doExplosionA()
	{
		...
		for (int i = 0; i < entities.size(); ++i)
		{
			...
			if (distance <= 1.0D)
			{
				...
				if (d3 != 0.0D)
				{
					if (FakePlayerUtils.cantDamage(this.getPlayer(), entity)) continue;
					...
					entity.attackEntityFrom(DamageSource.setExplosionSource(this), (float) ((int) ((d5 * d5 + d5) / 2.0D * 8.0D * (double) this.explosionSize + 1.0D)));
				...
				for (int k = 0; k < 16; ++k)
				{
					if (i == 0 || i == 15 || j == 0 || j == 15 || k == 0 || k == 15)
					{
						...
						for (float f = 0.3F; size > 0.0F; size -= f * 0.75F)
						{
							...
							{
								if (!FakePlayerUtils.cantBreak(x, y, z, this.getPlayer())) set.add(new ChunkPosition(x, y, z));
							}
	...
	{
		return this.player == null ? FakePlayerUtils.getModFake(this.worldObj) : this.player;
	}
